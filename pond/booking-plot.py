...
def filter_slots(
	...
	for slot, points in by_slot.items():
		...
		if dates and slot.date not in dates:
			continue
		if days_ahead is not None and base_date_text is not None:
			if date_offset(slot.date, base_date_text) >= days_ahead:
				continue
...
def plot_slots(by_slot: dict[SlotKey, list[tuple[datetime, int]]], venues: list[str] | None) -> None:
	...
	fig, axes = plt.subplots(
		nrows=len(locations),
		ncols=1,
		figsize=(10, max(4, 3.6 * len(locations))),
		sharex=True,
...
def main() -> None:
	...
	filtered_series = filter_slots(slot_series, venues=venues, days_ahead=args.days)
